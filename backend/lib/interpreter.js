...
class Interp {
  ...
  allocObject(className, ctorArgs, thread) {
    ...
    const ctor = decl.methods.find(m => m.isCtor);
  ...
  execStmt(stmt, scope, thread, frame) {
    ...
    switch (stmt.type) {
      ...
      case 'ExprStmt':
        this.evalExpr(stmt.expr, scope, thread);
        return null;
  ...
  evalExpr(node, scope, thread) {
    switch (node.type) {
      ...
      case 'Member': {
        ...
        if (obj === '__SYSTEM_OUT__') return obj; // unused path
      ...
      case 'NewArray': {
        const size = this.evalExpr(node.size, scope, thread);
  ...
  evalCall(node, scope, thread) {
    // System.out.println(...) used as expression
    if (node.callee.type === 'SystemOut' || (node.callee.type === 'Ident' && false)) {
      // handled by Print statement normally; fallback no-op
    }
    ...
    if (obj && obj.__heapRef) {
      ...
      return this.invoke(found.ownerClass, found.method, obj, args, thread);
    }

    // string/array builtin-ish helpers could go here
